...
func CreateSnippet(db *sql.DB, s *models.Snippet) (int64, error) {
	...
	var id int64
	err := db.QueryRow(
		`INSERT INTO snippets (user_id, title, content, language, is_public, share_slug)
		 VALUES ($1, $2, $3, $4, $5, $6)
		 RETURNING id, created_at, updated_at`,
		s.UserID, s.Title, s.Content, s.Language, s.IsPublic, nullableSlug(s.ShareSlug),
...
func GetSnippet(db *sql.DB, id, userID int64) (*models.Snippet, error) {
	...
	err := db.QueryRow(
		...
		&s.ShareSlug, &s.CreatedAt, &s.UpdatedAt)
...
func ListSnippets(db *sql.DB, userID int64, query, tag string) ([]models.Snippet, error) {
	...
	case tag != "":
		rows, err = db.Query(
			`SELECT s.id, s.user_id, s.title, s.content, s.language, s.is_public,
			        COALESCE(s.share_slug,''), s.created_at, s.updated_at
			 FROM snippets s
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &s.UserID, &s.Title, &s.Content, &s.Language,
			&s.IsPublic, &s.ShareSlug, &s.CreatedAt, &s.UpdatedAt); err != nil {
			return nil, err
		}
		s.Tags, _ = getTags(db, s.ID)
